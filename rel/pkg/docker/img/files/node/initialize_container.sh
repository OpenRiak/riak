...
if [ -d /var/run/riak ]; then
    ...
    if [ "${ENABLE_TLS:-false}" = "true" ]; then
        cp /root/ca/riakca.ca.crt.pem   /var/run/riak/
        cp /root/client/riakwriteclient* /var/run/riak/ 2>/dev/null || true
        cp /root/client/riakreadclient*  /var/run/riak/ 2>/dev/null || true
    fi
fi
