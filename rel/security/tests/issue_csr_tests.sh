...
# -------------------------------------------------------------------
#
#   A script to test issue_csr functionality.
#

ROOT_DIR=$(cd "$(dirname $0)"/.. && pwd)
TEST_PROGRAM_NAME="$ROOT_DIR/bin/issue_csr"
...
"$TEST_PROGRAM_NAME" -f "$NAME" -o "$OUT_DIR" > /dev/null
if [[ $? != 0 ]]; then
...
set +e  # Disable exiting on error
"$TEST_PROGRAM_NAME" -n "$NAME" -f "$NAME" > /dev/null
if [[ $? != 0 ]]; then
  echo -e "\tTest passed: Script exited with an error"
else
  echo -e "\tTest failed: Script did not exit with an error"
...
set +e  # Disable exiting on error
"$TEST_PROGRAM_NAME" -n "$NAME" -f "$NAME" -o /tmp/output_file > /dev/null
if [[ $? != 0 ]]; then
  echo -e "\tTest passed: Script exited with an error"
else
  echo -e "\tTest failed: Script did not exit with an error"
...
"$TEST_PROGRAM_NAME" -n "$NAME" -f "$NAME" -o "$OUT_DIR" > /dev/null
