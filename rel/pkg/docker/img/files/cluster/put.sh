...
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <user> <bucket> <key> <value>"
    exit 1
fi
...
echo curl -X PUT ${CURL_TLS_OPT} -u"$1:XXXXXX" "${PROTO}://localhost:10018/buckets/$2/keys/$3" -H "Content-Type: text/plain" -d "'$4'"
curl -X PUT ${CURL_TLS_OPT} -u"$1:123456" "${PROTO}://localhost:10018/buckets/$2/keys/$3" -H "Content-Type: text/plain" -d "'$4'"
echo